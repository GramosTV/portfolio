...
const Footer = () => {
  ...
  return (
    <motion.footer
      ...
      className="bg-slate-900 text-slate-400 p-8 md:p-12 shadow-inner relative overflow-hidden"
    >
      ...
      <div className="container mx-auto relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 items-center">
          ...
          >
            <h3 className="text-xl font-semibold text-slate-200 mb-2">Mikołaj Gramowski</h3>
            <p className="text-sm">{t('tagline')}</p>
          </motion.div>

          <motion.div
            className="md:col-span-2 flex flex-col sm:flex-row justify-center md:justify-end items-center space-y-4 sm:space-y-0 sm:space-x-6"
          ...
          >
            {socialLinks.map((link) => (
              ...
              >
                {link.icon}
                <span className="ml-2 text-sm group-hover:underline">{t(link.labelKey)}</span>
        ...
        <motion.div
          className="border-t border-slate-700 pt-8 text-center text-sm"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <p className="text-xs text-slate-500 mt-1">
            {t('builtWith')}
            <Link href="/privacy-policy" className="ml-2 hover:text-sky-400 transition-colors">
              {t('privacyPolicy')}
            </Link>
            {' | '}
            <Link href="/terms-of-service" className="ml-1 hover:text-sky-400 transition-colors">
              {t('termsOfService')}
            </Link>
          </p>
        </motion.div>
